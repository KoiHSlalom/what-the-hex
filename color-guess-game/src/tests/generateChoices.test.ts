...
describe('generateChoices', () => {
    it('should return 2 choices for easy mode', () => {
        ...
        const choices = generateChoices(currentColor, Difficulty.EASY);
        expect(choices.length).toBe(2);
    });

    it('should return 4 choices for medium mode', () => {
        const currentColor = '#33ff57';
        const choices = generateChoices(currentColor, Difficulty.MEDIUM);
        expect(choices.length).toBe(4);
    });

    it('should return 6 choices for hard mode', () => {
        const currentColor = '#3357ff';
        const choices = generateChoices(currentColor, Difficulty.HARD);
